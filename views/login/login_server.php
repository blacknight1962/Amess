...
if (isset($_POST['id']) && isset($_POST['password'])) {
  ...
  if (empty($user_id)) {
    header('location:../views/main_index.php?error=아이디가 비어 있어요');
    exit();
  } else if (empty($user_pw)) {
    header('location:../views/main_index.php?error=비밀번호가 비어 있어요');
    exit();
  } else {
    ...
    if (mysqli_num_rows($result) === 1) {
      ...
      if (password_verify($user_pw, $hash)) {
        ...
        header('location: ../views/main_index.php');
        exit();
      } else {
        header('location: ../views/main_index.php?error=비밀번호 에러입니다. 다시 한번 확인 하십시오');
        exit();
      }
    } else {
      header('location: ../views/main_index.php?error=아이디 에러입니다. 다시 한번 확인 하십시오');
      exit();
    }
  }
} else {
  header('location: ../views/main_index.php?error=로그인 에러입니다. 관리자에게 문의 하십시오');
  exit();
}
?>